...
async def create_task(task_data: TaskCreate):
    ...
    x = TaskRead(id = x, title = task_data.title, description= task_data.description, completed = task_data.completed)
...
async def update_task(task_id: int, task_data: TaskCreate):
    ...
    conn.commit()
    conn.close()
    x = TaskRead(id = tid, title = task_data.title, description= task_data.description, completed = task_data.completed)
...
async def delete_task(task_id: int):
    ...
    return  {"message": f"Task {tid} was deleted successfully"}
